...
#include <iostream>
#include <string>
#define CONSOLEXPLIB_DLL "ConsoleXP.dll"
...
int main(int argc, char** argv)
{
    ...
    if (argc < 2) {
        result = MessageBoxA(0,
            "This tool will patch your game client to enable ConsoleXP features.\n"
            "Make sure you have a backup of your game client before proceeding.\n"
            "Press OK to continue or Cancel to exit.", s_appName.c_str(), MB_OKCANCEL | MB_ICONINFORMATION);
    }
    ...
    if (result == IDOK)
    {
        ...
        std::filesystem::path libInGamePath = std::filesystem::path(exePath).parent_path() / CONSOLEXPLIB_DLL;
        std::filesystem::path libInAppPath = std::filesystem::absolute(argv[0]).parent_path() / CONSOLEXPLIB_DLL;
        ...
        if (std::filesystem::is_regular_file(libInGamePath)) {
            ...
            message(MB_ICONWARNING,
                "Patch succesfully applied\n"
                "But look like `" CONSOLEXPLIB_DLL "` is missing\n"
